...
def return_coords(X_input_data=None):
	...
	for pdq_identifier in list(data.keys()):
		...
		output_str+="google.maps.event.addListener(pdqPolygon"+str(counter)+",\"click\",function() {							$.get(\"/get_pdq_data/"+str(pdq_identifier)+".0\", function(data) {console.log($.parseJSON(data));myBarChart.data.datasets[0].data = $.parseJSON(data);myBarChart.update();document.getElementById(\"p1\").innerHTML = \""+pdq_identifier+"\";document.getElementById(\"pdq_num\").innerHTML = \""+pdq_identifier+"\";document.getElementById(\"pdq_num1\").innerHTML = \""+pdq_identifier+"\";document.getElementById(\"redirect_pdq_link\").href=\"https://spvm.qc.ca/en/pdq"+str(int(pdq_identifier))+"\";})});"
		counter += 1
	return output_str
